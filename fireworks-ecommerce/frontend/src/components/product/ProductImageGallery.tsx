...
export default function ProductImageGallery({ images, name }: Props) {
  ...
  const list = images.length
    ? images
    : [{ url: "https://placehold.co/600x600?text=No+Image", public_id: "", alt: `${name} image` }];

  return (
    <div>
      {/* Main image */}
      <div className="relative rounded-2xl overflow-hidden aspect-square bg-gray-50 group cursor-zoom-in" onClick={() => setZoomed(true)}>
        <img src={list[current].url} alt={list[current].alt || name} className="w-full h-full object-contain" />
      ...
      {list.length > 1 && (
        <div className="flex gap-2 mt-3 overflow-x-auto pb-1">
          {list.map((img, i) => (
            ...
            >
              <img src={img.url} alt={img.alt || `${name} thumbnail ${i + 1}`} className="w-full h-full object-cover" />
            </button>
          ))}
        </div>
      )}
      ...
      {zoomed && (
        <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4" onClick={() => setZoomed(false)}>
          <img src={list[current].url} alt={list[current].alt || name} className="max-w-full max-h-full object-contain" />
        </div>
      )}
    </div>
  );
