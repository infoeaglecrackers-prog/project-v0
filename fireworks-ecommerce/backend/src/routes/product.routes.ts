...
router.post(
  ...
  [
    ...
    // Description is optional now; ignore empty strings and validate length only when provided
    body("description").optional({ checkFalsy: true }).trim().isLength({ min: 10 }).withMessage("Description must be at least 10 characters if provided"),
